    ...
    class HTTPBasicFilter(QgsServerFilter):
        ...
        def responseComplete(self):
            ...
            if self.serverInterface().getEnv('HTTP_AUTHORIZATION'):
                ...
                if (username.decode('utf-8') == QGIS_SERVER_USERNAME and
                        password.decode('utf-8') == QGIS_SERVER_PASSWORD):
                    return
            # No auth ...
            request.clear()
...
if QGIS_SERVER_OAUTH2_AUTH:
    ...
    class OAuth2Filter(QgsServerFilter):
        ...
        def responseComplete(self):
            ...
            # Issue a new token
            if self.serverInterface().getEnv('REQUEST_URI').find('/token') != -1:
                _token(ttl)
                return

            # Refresh token
            if self.serverInterface().getEnv('REQUEST_URI').find('/refresh') != -1:
                _token(ttl)
                return

            # Check for valid token
            if self.serverInterface().getEnv('HTTP_AUTHORIZATION'):
                result, response = oauth_server.verify_request(self.serverInterface().getEnv('REQUEST_URI'), 'post', '', {'Authorization': self.serverInterface().getEnv('HTTP_AUTHORIZATION')})
                if result:
                    # This is a test endpoint for OAuth2, it requires a valid token
                    if self.serverInterface().getEnv('REQUEST_URI').find('/result') != -1:
                        request.clear()
                        request.appendBody(b'Valid Token: enjoy OAuth2')
                    # Standard flow
                    return
                else:
            ...
            request.appendBody(b'Invalid Token: Authorization required.')
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        url = ('https://' if HTTPS_ENABLED else 'http://') + self.address_string() + ':' + str(self.server.server_port) + self.path
        qgs_server.putenv('REQUEST_URI', url)
        headers, body = qgs_server.handleRequest(url)
    ...
    def do_POST(self):
        ...
        qgs_server.putenv('REQUEST_BODY', post_body)
        self.path = self.path + '&REQUEST_BODY=' + \
            post_body.replace('&amp;', '')
        # Add REQUEST=Transaction to WFS-T requests
        if self.path.find('SERVICE=WFS') != -1:
